...
export default function ProductsPage() {
  ...
  useEffect(() => {
    if (params.id) {
      const prodByCategory = productsSel.filter(
    ...
    <div className={s.wrapper}>
      ...
      <div className={s.filterBar}>
        <div className={s.filterPrice}>
          ...
          <input
            onChange={(e) => dispatch(filterByPriceFrom(+e.target.value))}
            name="priceValueFrom"
            type="number"
            placeholder="from"
          />
          <input
            onChange={(e) => dispatch(filterByPriceTo(+e.target.value))}
            name="priceValueTo"
            type="number"
            placeholder="to"
          />
        ...
        {params.sale === undefined ? (
          <div>
            ...
            <input
              onChange={() => checkboxOnChange(!checkBoxStatus)}
              type="checkbox"
        ...
        <div className={s.filterSorted}>
          ...
          <select onChange={sortOnChange} name="price">
            <option value="1">price from low</option>
            <option value="2">price from high</option>
          </select>
        </div>
      </div>
